...
def build_quality_report(
    ...
    visible = max(from_segment, int(visible))
    processed_segments = connection.execute(
        """
        SELECT COUNT(DISTINCT result.segment_id)
        FROM segment_results result
    ...
    unresolved_merges = connection.execute(
        """
        SELECT COUNT(*) FROM (
            SELECT candidate.left_entity_id, candidate.right_entity_id
            ...
            WHERE candidate.book_id = ? AND candidate.status IN ('unreviewed', 'needs_review')
              AND left_entity.first_segment <= ? AND right_entity.first_segment <= ?
    ...
    contradiction_rows = connection.execute(
        "SELECT left_type, left_id, right_type, right_id FROM contradictions WHERE book_id = ? AND status = 'unreviewed'",
        (book_id,),
    ).fetchall()
    target_tables = {
        "entity": "entities", "claim": "claims", "event": "events",
        "place_relation": "place_relations", "world_note": "world_notes", "entry": "entries",
    }
    ...
    contradictions = sum(
        1 for row in contradiction_rows
        if all(
            first is None or first <= visible
            for first in (
                _target_first_segment(str(row["left_type"]), int(row["left_id"])),
                _target_first_segment(str(row["right_type"]), int(row["right_id"])),
            )
        )
    )
    ...
    benchmark = connection.execute(
        ...
        WHERE book_id = ? AND source_segment <= ? AND confirmed_by_user = 1
    ...
    return {
        ...
        "internal_evaluation_gate_passed": accuracy_gate_passed,
