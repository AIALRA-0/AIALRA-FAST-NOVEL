...
CREATE TABLE IF NOT EXISTS books (
    ...
    report_language TEXT NOT NULL DEFAULT 'follow_source',
...
CREATE TABLE IF NOT EXISTS segment_versions (
    ...
    book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,
    segment_id INTEGER,
    update_id INTEGER REFERENCES book_update_batches(id) ON DELETE SET NULL,
    ...
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(book_id, segment_id, revision)
);
...
CREATE TABLE IF NOT EXISTS story_worlds (
    ...
    created_by TEXT NOT NULL DEFAULT 'local_partition',
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(book_id, name)
);
...
CREATE TABLE IF NOT EXISTS narrative_units (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,
    world_id INTEGER NOT NULL REFERENCES story_worlds(id) ON DELETE CASCADE,
    title TEXT NOT NULL,
    ...
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(book_id, start_segment, end_segment)
);

CREATE TABLE IF NOT EXISTS narrative_unit_links (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,
    source_unit_id INTEGER NOT NULL REFERENCES narrative_units(id) ON DELETE CASCADE,
    target_unit_id INTEGER NOT NULL REFERENCES narrative_units(id) ON DELETE CASCADE,
    relation TEXT NOT NULL,
    evidence_json TEXT NOT NULL DEFAULT '[]',
    confidence REAL NOT NULL DEFAULT 0.5,
    created_by TEXT NOT NULL DEFAULT 'local_partition',
    UNIQUE(book_id, source_unit_id, target_unit_id, relation)
);
...
CREATE TABLE IF NOT EXISTS review_tasks (
    ...
    evidence_json TEXT NOT NULL DEFAULT '[]',
    rebuild_scope_json TEXT NOT NULL DEFAULT '[]',
...
CREATE TABLE IF NOT EXISTS ui_issues (
    ...
    verified_environment TEXT NOT NULL DEFAULT '',
...
CREATE INDEX IF NOT EXISTS idx_segment_versions_book ON segment_versions(book_id, segment_id, revision);
CREATE INDEX IF NOT EXISTS idx_story_worlds_book ON story_worlds(book_id, status, id);
CREATE INDEX IF NOT EXISTS idx_narrative_units_book ON narrative_units(book_id, start_segment, end_segment);
CREATE INDEX IF NOT EXISTS idx_review_tasks_book ON review_tasks(book_id, status, severity, category);
...
def initialize(path: Path) -> None:
    ...
    with connect(path) as connection:
        ...
        _migrate_v293(connection)
        _migrate_v297(connection)
        _migrate_v298(connection)
...
def _migrate_v293(connection: sqlite3.Connection) -> None:
    """Add reversible story partition, incremental update, review, and editing foundations."""

    segment_columns = {str(row[1]) for row in connection.execute("PRAGMA table_info(segments)")}
    for definition in (
        ...
        if column not in segment_columns:
            connection.execute(f"ALTER TABLE segments ADD COLUMN {definition}")
    update_columns = {str(row[1]) for row in connection.execute("PRAGMA table_info(book_update_batches)")}
    for definition in (
        "match_summary_json TEXT NOT NULL DEFAULT '{}'",
        "affected_scope_json TEXT NOT NULL DEFAULT '{}'",
        "reuse_ratio REAL NOT NULL DEFAULT 0",
    ):
        column = definition.split()[0]
        if column not in update_columns:
            connection.execute(f"ALTER TABLE book_update_batches ADD COLUMN {definition}")
    for segment in connection.execute(
        "SELECT id, text, content_hash, stable_key FROM segments WHERE content_hash = '' OR stable_key = ''"
    ).fetchall():
        connection.execute(
            "UPDATE segments SET content_hash = ?, stable_key = ? WHERE id = ?",
            (
                str(segment["content_hash"]) or hashlib.sha256(str(segment["text"]).encode("utf-8")).hexdigest(),
                str(segment["stable_key"]) or f"segment-{int(segment['id'])}",
                int(segment["id"]),
            ),
        )
    features = (
        "atlas_geometry_v2", "atlas_volume_v2", "narrative_partition_v1",
        "incremental_merge_v2", "review_center_v2", "semantic_editor_v2",
    )
    for book in connection.execute("SELECT id FROM books"):
        for feature in features:
            connection.execute(
...
def _migrate_v297(connection: sqlite3.Connection) -> None:
    """Add measurable UI issue provenance without changing existing records."""

    existing = {str(row[1]) for row in connection.execute("PRAGMA table_info(ui_issues)")}
    for definition in (
        "component_key TEXT NOT NULL DEFAULT ''",
        "issue_class TEXT NOT NULL DEFAULT ''",
        "root_cause TEXT NOT NULL DEFAULT ''",
        ...
        column = definition.split()[0]
        if column not in existing:
            connection.execute(f"ALTER TABLE ui_issues ADD COLUMN {definition}")


def _migrate_v298(connection: sqlite3.Connection) -> None:
    """补齐阅读窗口、线索可执行性和发布版本字段；旧数据原样保留。"""

    existing = {str(row[1]) for row in connection.execute("PRAGMA table_info(open_threads)")}
    for definition in (
        "actionability TEXT NOT NULL DEFAULT 'informational'",
        "resolution_route TEXT NOT NULL DEFAULT ''",
        "evidence_segment_ids_json TEXT NOT NULL DEFAULT '[]'",
        "recommended_action TEXT NOT NULL DEFAULT ''",
    ):
        column = definition.split()[0]
        if column not in existing:
            connection.execute(f"ALTER TABLE open_threads ADD COLUMN {definition}")
    # 同一套状态开关用于前端和派生视图，迁移只增加记录，不删除旧开关
    for book in connection.execute("SELECT id FROM books"):
        for feature in ("reading_window_v1", "provider_probe_v1", "loading_progress_v1"):
            connection.execute(
                "INSERT OR IGNORE INTO feature_flags(book_id, feature_key, enabled) VALUES (?, ?, 1)",
                (int(book["id"]), feature),
            )
