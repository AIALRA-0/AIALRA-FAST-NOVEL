...
class BookPatch(BaseModel):
    ...
    report_language: Literal["follow_source", "zh-CN", "en"] | None = None
...
class BookUpdateResolution(BaseModel):
    ...
    action: Literal["keep_current", "apply_incremental", "import_as_new", "auto"]
...
class StoryWorldMergeRequest(BaseModel):
    """把多个故事世界合并为一个人工确认世界。"""

    world_ids: list[int] = Field(min_length=2)
    name: str = Field(min_length=1, max_length=160)


class StoryWorldSplitRequest(BaseModel):
    """把选定故事单元拆成一个新的、仍属于同一本书的世界分区。"""

    unit_ids: list[int] = Field(min_length=1)
    name: str = Field(min_length=1, max_length=160)


class NarrativeUnitMoveRequest(BaseModel):
    """把故事单元移动到另一个世界。"""

    world_id: int = Field(gt=0)


class ReviewTaskPatch(BaseModel):
    """处理一个统一审核任务。"""

    action: Literal["accept_suggestion", "keep_separate", "defer", "resolved"]
...
class RecordPatch(BaseModel):
    ...
    field_name: Literal[
        "name", "title", "summary", "temporal_value", "category", "status", "attributes",
        "aliases", "kind", "importance", "location_entity_id", "transport_mode", "participants",
        "source_entity_id", "target_entity_id", "predicate", "directionality", "reverse_predicate",
        "relative_position", "container_entity_id", "contained_entity_id", "temporal_scope",
        "start_segment", "end_segment", "world_id",
    ]
...
class UiIssueCreate(BaseModel):
    ...
    component_key: str = Field(default="", max_length=160)
    issue_class: str = Field(default="", max_length=120)
    viewport: str = Field(default="", max_length=120)
    detected_environment: str = Field(default="", max_length=500)
    verified_environment: str = Field(default="", max_length=500)
    severity: Literal["low", "medium", "high", "blocker"]
    summary: str = Field(min_length=2, max_length=500)
    root_cause: str = Field(default="", max_length=5_000)
    ...
    regression_case: str = Field(default="", max_length=5_000)
...
class UiIssuePatch(BaseModel):
    ...
    component_key: str | None = Field(default=None, max_length=160)
    issue_class: str | None = Field(default=None, max_length=120)
    detected_environment: str | None = Field(default=None, max_length=500)
    verified_environment: str | None = Field(default=None, max_length=500)
    ...
    regression_case: str | None = Field(default=None, max_length=5_000)
