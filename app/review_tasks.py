...
def _task_first_segment(
    connection: sqlite3.Connection,
    source_type: str,
    source_id: int | None,
) -> int | None:
    """Resolve a task's earliest source segment without exposing future facts."""
    ...
    queries: dict[str, tuple[str, tuple[Any, ...]]] = {
        "merge_candidate": (
            """
            SELECT MIN(entity.first_segment) AS first_segment
            FROM entity_merge_candidates candidate
            JOIN entities entity ON entity.id IN (candidate.left_entity_id, candidate.right_entity_id)
            WHERE candidate.id = ?
            """,
            (source_id,),
        ),
        "connectivity": (
            """
            SELECT entity.first_segment AS first_segment
        ...
        "event_location": (
            "SELECT first_segment FROM events WHERE id = ?",
            (source_id,),
        ),
        "time_edge": (
            """
            SELECT MIN(event.first_segment) AS first_segment
            FROM event_order_edges edge
            JOIN events event ON event.id IN (edge.earlier_event_id, edge.later_event_id)
            WHERE edge.id = ?
            """,
            (source_id,),
        ),
    ...
    *,
    ...
    for row in rows:
        ...
        if through_segment is not None:
            first_segment = _task_first_segment(
                connection, str(item.get("source_type") or ""),
                int(item["source_id"]) if item.get("source_id") is not None else None,
            )
            # Tasks without a source are retained; source-backed tasks past the
            # spoiler ceiling are not allowed to leak into the current window.
            if first_segment is not None and first_segment > through_segment:
                continue
            if first_segment is not None and first_segment < from_segment:
