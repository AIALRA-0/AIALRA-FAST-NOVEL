...
def systems_payload(connection: sqlite3.Connection, book_id: int) -> list[dict[str, Any]]:
...
def story_knowledge_context(
    ...
    if entity_ids:
        ...
        rows = connection.execute(
            ...
            SELECT entity.id AS entity_id, entity.name, entity.kind, concept.id AS concept_id,
                   ...
                   claim.source_kind, claim.confidence, COUNT(link.evidence_id) AS evidence_count
    ...
    systems = []
    for system in systems_payload(connection, book_id):
        visible_nodes = [
            node for node in system["nodes"]
            if int(node["effective_from_segment"] or 0) <= through_segment
            and (node["effective_to_segment"] is None or int(node["effective_to_segment"]) >= through_segment)
        ]
        if visible_nodes:
