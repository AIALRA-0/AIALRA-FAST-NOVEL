...
app = FastAPI(title="小说证据图谱", version="2.9.4-rc.1", lifespan=lifespan)
...
def providers() -> list[dict[str, Any]]:
    ...
    return [
        {"id": "auto", "label": "自动选择已达标模型", "available": True, "note": "质量优先，再比较费用和速度"},
        {"id": "mock", "label": "离线证据演示", "available": True, "note": "不调用云端模型"},
        {"id": "deepseek", "label": "DeepSeek 开放平台", "available": bool(settings.deepseek_api_key), "note": settings.deepseek_model},
        {"id": "moonshot", "label": "Moonshot 开放平台", "available": bool(settings.moonshot_api_key), "note": "需要开放平台密钥，不接受 Kimi Code 订阅凭据"},
        {
            "id": "codex_luna", "label": "Codex Luna · 本机 ChatGPT 登录",
            "available": bool(codex["available"]), "note": codex["message"],
...
def narrative_structure(book_id: int) -> dict[str, Any]:
    ...
    with transaction(settings.database_path) as connection:
        ...
        return get_narrative_structure(connection, book_id) if existing else rebuild_narrative_structure(connection, book_id)
...
def book_review_tasks(book_id: int, include_resolved: bool = Query(default=False)) -> list[dict[str, Any]]:
    with transaction(settings.database_path) as connection:
        ensure_book(connection, book_id)
        sync_review_tasks(connection, book_id)
        return list_review_tasks(connection, book_id, include_resolved=include_resolved)
...
def control_plane_overview(book_id: int) -> dict[str, Any]:
    ...
    with connect(settings.database_path) as connection:
        ...
        benchmark = evaluate_benchmarks(connection, book_id)
        global_evaluation = evaluation_progress(connection)
    ...
    return {
        ...
        "model_routes": routes,
        "benchmark": benchmark,
        "evaluation_progress": global_evaluation,
...
def overview(book_id: int, through_segment: int = Query(default=1_000_000, ge=0)) -> dict[str, Any]:
    """返回同一剧透边界内的全部派生视图。"""

    with connect(settings.database_path) as connection:
        book = ensure_book(connection, book_id)
        visible = min(through_segment, max(0, int(book["segment_count"]) - 1))
        raw_entities = rows(
            connection.execute(
                """
        ...
        processed_segments = connection.execute(
            """
            SELECT COUNT(DISTINCT segment_id) FROM segment_results
            WHERE book_id = ? AND (provider = 'demo' OR prompt_version = ?)
            """,
            (book_id, PROMPT_VERSION),
        ).fetchone()[0]
        quality = build_quality_report(connection, book_id, visible)
        ...
        segment_titles = rows(
            connection.execute(
                "SELECT id, ordinal, chapter_title, anchor FROM segments WHERE book_id = ? ORDER BY ordinal",
                (book_id,),
            ).fetchall()
        )
    return {
...
def search_book(
    ...
    through_segment: int = Query(default=1_000_000, ge=0),
) -> list[dict[str, Any]]:
    """同时搜索结构知识和原文片段。"""
    ...
    with connect(settings.database_path) as connection:
        ...
        for target_type, table, title_expression, searchable in (
            ...
            records = connection.execute(
                ...
                FROM {table} WHERE book_id = ? AND first_segment <= ?{active_filter}
                  AND {searchable} LIKE ? LIMIT ?
                """,  # noqa: S608
                (book_id, visible, pattern, remaining),
            ).fetchall()
            results.extend(
                {
        ...
        if remaining > 0:
            segments = connection.execute(
                ...
                WHERE book_id = ? AND ordinal <= ? AND text LIKE ? ORDER BY ordinal LIMIT ?
                """,
                (book_id, visible, pattern, remaining),
            ).fetchall()
...
def segment(segment_id: int) -> dict[str, Any]:
    """读取完整原文片段，供证据抽屉跳转。"""
...
def health() -> dict[str, str]:
    ...
    return {"status": "ok", "version": "2.9.4-rc.1"}
...
def ready() -> dict[str, str]:
    ...
    return {"status": "ready", "version": "2.9.4-rc.1"}
