...
def patch_record(target_type: str, target_id: int, patch: RecordPatch) -> dict[str, Any]:
    ...
    table_map = {
        "entity": ("entities", {"name", "summary"}),
        "event": ("events", {"title", "summary", "temporal_value"}),
        "world_note": ("world_notes", {"title", "summary", "category"}),
        "entry": ("entries", {"name", "summary", "category", "attributes"}),
        "claim": ("claims", {"summary", "status"}),
    }
    ...
    table = table_info[0]
    database_field = "attributes_json" if patch.field_name == "attributes" else patch.field_name
    with transaction(settings.database_path) as connection:
        ...
        old_value = str(record[database_field])
        ...
        connection.execute(
            ...
            (
                ...
                old_value, new_value, patch.reason,
            ),
        )
        connection.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, 'human', ?)
            """,
            (record["book_id"], target_type, target_id, patch.field_name, new_value, patch.reason),
        )
