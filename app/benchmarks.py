...
def evaluate_benchmarks(connection: sqlite3.Connection, book_id: int) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_passed": bool(total >= 20 and accuracy is not None and accuracy >= 95 and critical_failed == 0),
    }
...
def evaluation_progress(connection: sqlite3.Connection, *, refresh: bool = True) -> dict[str, Any]:
    ...
    books_below_minimum = sum(1 for count in per_book_counts.values() if count < 20)
    if refresh:
    ...
    return {
        ...
        "remaining_books": max(0, 5 - len(book_ids)),
    }
