...
LAYOUT_VERSION = "semantic-atlas-v2.9.3-occupancy-volume1"
_DIRECTION_VECTORS = {
...
def _map_display_name(name: str) -> str:
    """Use the same short label rule as the browser map."""

    characters = list(str(name or ""))
    return "".join(characters[:10]) + "…" if len(characters) > 12 else "".join(characters)
...
def _node_label_placements(
    ...
    for place in ordered:
        ...
        width = max(52.0, len(_map_display_name(str(place["name"]))) * 14.0 + 18.0)
        candidates = [
        ...
        placements[node_id] = {
            ...
            "width": round(width, 2), "visible": selected_score == 0,
