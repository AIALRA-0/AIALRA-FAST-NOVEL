...
当前版本为 `2.9.0-rc.1` 候选版，这是一个可本地运行的个人项目，不是已经承诺服务等级的多租户平台
...
- 121 项 Python 测试通过，1 项按环境条件跳过
- 6 条隔离合成数据 Chromium 主流程通过，3 条真实作品流程在隔离库中按条件跳过
- 2 条《西游记》真实分析数据流程通过，覆盖 1,049 个编年步骤、157 个地点、45 个语义区域和 40 条双向关系
- 1 条五部公版全文书库流程通过，覆盖检索、切换和打开
- JavaScript 语法检查与当前状态合同检查通过
- 浏览器覆盖快速连续前进、2D/3D 同步、三维区域网格、末步结束、长篇标签碰撞、窄屏操作、统一选择器、防剧透输入和知识库检索

真实长篇验收数据库和截图只留在本机，不进入公开仓库

这些数字只证明固定样本，不表示任意小说天然达到 95% 准确率；当前只有 28/300 条人工金标准，候选版不会冒充已经通过正式质量门
