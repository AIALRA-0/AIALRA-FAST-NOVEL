...
当前版本为 `2.9.2-rc.1` 候选版，这是一个可本地运行的个人项目，不是已经承诺服务等级的多租户平台
...
| 语义世界图 | 2D、3D、播放条和故事卡共用唯一的 `StoryMapStep`，人物移动会同步突出当前区域 | 彩色区域只整理有证据的包含关系或故事联系，不代表原文明示国界或地貌 |
...
区域名称来自原文容器或区域内的代表地点，标签放在边界外侧并用短线指回区域，不再使用编号占位

三维导览沿用相同的 X/Y，Z 只表示有证据的地域包含深度，不代表海拔；当前路线、区域和编年步骤保持同步，播放可选 `0.5×`、`1×`、`1.5×` 和 `2×`
