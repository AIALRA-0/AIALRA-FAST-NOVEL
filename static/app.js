...
function renderMap() {
  ...
  const semanticRegions = state.mapPresentation === "atlas" ? (state.mapLayout?.regions || []).map((region, index) => {
    ...
    const label = Number.isFinite(Number(labelAnchor.x)) && Number.isFinite(Number(labelAnchor.y))
      ? `<g class="semantic-region-label-wrap" data-placement="${escapeHtml(labelAnchor.placement_mode || "side_lane")}">${connector.hidden ? "" : `<line class="semantic-region-label-connector" x1="${Number(connector.x1 ?? labelAnchor.x)}" y1="${Number(connector.y1 ?? labelAnchor.y)}" x2="${Number(connector.x2 ?? labelAnchor.x)}" y2="${Number(connector.y2 ?? labelAnchor.y)}"></line>`}<rect class="semantic-region-label-bg" x="${Number(labelAnchor.x) - labelWidth / 2}" y="${Number(labelAnchor.y) - 16}" width="${labelWidth}" height="25" rx="8"></rect><text class="semantic-region-label" x="${Number(labelAnchor.x)}" y="${Number(labelAnchor.y) + 1}">${escapeHtml(regionName)}</text></g>`
      : "";
