...
function readingWindowPreference(bookId = state.bookId) {
...
function resetMapStateForBook() {
  ...
  // 切书时先失效所有派生数据；异步请求完成前不能继续展示上一本书的知识条目
  state.overview = null;
  state.mapLayout = null;
  state.narrativeMemory = null;
...
function renderBookSwitchPlaceholder() {
  // 知识库需要立即清空旧作品的行，其他页面由统一加载条覆盖并在请求结束后重绘
...
async function initialize() {
  const stages = ["读取书库", "读取文件夹", "检查模型连接"];
  let completed = 0;
  try {
    renderLoadingProgress(stages[0], completed, stages.length);
    state.books = await api("/api/books");
    completed += 1;
    renderLoadingProgress(stages[1], completed, stages.length);
    state.folders = await api("/api/library/folders");
    completed += 1;
    renderLoadingProgress(stages[2], completed, stages.length);
    ...
    renderLoadingProgress("初始化失败", completed, stages.length, error.message);
  }
}
...
function renderSidebarLibraryTree() {
  ...
  const booksMarkup = (folderId, depth = 0) => (booksByFolder.get(String(folderId)) || []).map((book) => `<button class="sidebar-book${Number(state.bookId) === Number(book.id) ? " active" : ""}" style="--tree-depth:${depth + 1}" data-book="${book.id}" type="button" title="${escapeHtml(book.title)}"><span aria-hidden="true">▤</span><span>${escapeHtml(book.title)}</span></button>`).join("");
  const folderMarkup = (parent = "root", depth = 0, trail = new Set()) => (children.get(String(parent)) || []).map((folder) => {
    ...
    return `<div class="sidebar-folder" style="--tree-depth:${depth}"><button class="sidebar-folder-toggle" data-folder="${folder.id}" aria-expanded="${open}" type="button"><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M6 4.5 10 8l-4 3.5"/></svg><span>${escapeHtml(folder.name)}</span><small>${Number(folder.book_count || 0)}</small></button><div class="sidebar-folder-children" ${open ? "" : "hidden"}>${booksMarkup(folder.id, depth)}${folderMarkup(folder.id, depth + 1, nextTrail)}</div></div>`;
  }).join("");
...
function renderProviderOptions() {
  $("#provider-select").innerHTML = state.providers.map((provider) => {
    const suffix = provider.available ? "" : ` · ${providerStatusLabel(provider.status)}`;
  ...
  const statusHost = $("#provider-status");
  if (!statusHost) return;
  statusHost.innerHTML = state.providers
    .filter((provider) => !["auto", "mock"].includes(provider.id))
    .map((provider) => {
      const status = providerStatusLabel(provider.status);
      const cls = provider.status === "connected" ? "is-ready" : ["unconfigured", "configured"].includes(provider.status) ? "" : "is-error";
      const action = provider.status === "unconfigured" ? "配置密钥" : "重新测试";
      return `<div class="provider-status-card ${cls}"><div><strong>${escapeHtml(provider.label)}</strong><small>${escapeHtml(status)}${provider.model ? ` · ${escapeHtml(provider.model)}` : ""}</small></div><button type="button" class="button button-quiet provider-probe" data-provider="${escapeHtml(provider.id)}">${action}</button></div>`;
    }).join("");
  statusHost.querySelectorAll(".provider-probe").forEach((button) => button.addEventListener("click", () => {
    if (state.providers.find((item) => item.id === button.dataset.provider)?.status === "unconfigured") {
      const provider = button.dataset.provider;
      if ($("#key-provider")) $("#key-provider").value = provider;
      openDialog("#key-dialog", "#provider-key");
      $("#provider-key")?.focus();
      return;
    }
    probeProvider(button.dataset.provider);
  }));
}

function providerStatusLabel(status) {
  return ({
    ready: "已连接", local: "本地演示", configured: "已配置，尚未测试", connected: "已连接",
    unconfigured: "未配置", auth_failed: "认证失败", timeout: "连接超时",
    service_error: "服务异常", model_unavailable: "模型不可用",
  })[status] || "待测试";
}

async function probeProvider(providerId) {
  const button = $(`.provider-probe[data-provider="${CSS.escape(providerId)}"]`);
  if (button) { button.disabled = true; button.textContent = "测试中…"; }
  try {
    const result = await api(`/api/providers/${encodeURIComponent(providerId)}/probe`, { method: "POST" });
    const current = state.providers.find((provider) => provider.id === providerId);
    if (current) Object.assign(current, result, { available: result.status === "connected" });
    renderProviderOptions();
    toast(`${current?.label || providerId}：${providerStatusLabel(result.status)}`);
  } catch (error) {
    toast(error.message, true);
  } finally {
...
function renderLoadingProgress(stage, completed, total, error = "") {
  ...
  let host = panel.querySelector(".loading-progress-banner");
  if (!host) {
    if (!state.overview) panel.innerHTML = "";
    host = document.createElement("div");
    host.className = state.overview ? "loading-progress-banner" : "loading";
    panel.prepend(host);
  }
  const percentage = Math.max(0, Math.min(100, Math.round((completed / Math.max(1, total)) * 100)));
  host.innerHTML = `<div class="loading-progress"><div class="loading-progress-row"><strong>${escapeHtml(stage)}</strong><span>${percentage}%</span></div><div class="loading-progress-bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${percentage}"><span style="width:${percentage}%"></span></div><small>${completed}/${total} 个阶段已完成</small>${error ? `<div class="loading-progress-error">${escapeHtml(error)}</div><button class="button button-quiet loading-retry" type="button">重试</button>` : ""}</div>`;
  const retry = host.querySelector(".loading-retry");
  if (retry) retry.addEventListener("click", () => loadOverview(state.readingThrough, false, state.readingFrom));
}

function clearLoadingProgress() {
  state.loadingProgress = null;
  $("#view-panel")?.querySelector(".loading-progress-banner")?.remove();
}

function renderInlineLoading(host, stage, completed = 0, total = 1, error = "", retry = null) {
  if (!host) return;
  const percentage = Math.max(0, Math.min(100, Math.round((completed / Math.max(1, total)) * 100)));
  host.innerHTML = `<div class="loading-progress inline-loading-progress"><div class="loading-progress-row"><strong>${escapeHtml(stage)}</strong><span>${percentage}%</span></div><div class="loading-progress-bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${percentage}"><span style="width:${percentage}%"></span></div><small>${completed}/${total} 个阶段已完成</small>${error ? `<div class="loading-progress-error">${escapeHtml(error)}</div>${retry ? '<button class="button button-quiet inline-loading-retry" type="button">重试</button>' : ""}` : ""}</div>`;
  if (retry) host.querySelector(".inline-loading-retry")?.addEventListener("click", retry);
}

async function loadOverview(throughSegment = null, silent = false, fromSegment = null) {
  ...
  const stages = [
    ...
    ["整理地图布局", `/api/books/${requestedBookId}/map-layout${query}`],
    ["整理叙事记忆", `/api/books/${requestedBookId}/narrative-memory${query}`],
    ["读取知识分类", `/api/books/${requestedBookId}/knowledge-facets${query}`],
    ["读取知识概念", `/api/books/${requestedBookId}/concepts?status=&limit=1000${querySuffix}`],
    ["读取体系图谱", `/api/books/${requestedBookId}/systems${query}`],
    ["读取待处理事项", `/api/books/${requestedBookId}/review-tasks${query}`],
    ["读取故事范围", `/api/books/${requestedBookId}/narrative-structure${query}`],
  ];
  const values = [];
  try {
    for (let index = 0; index < stages.length; index += 1) {
      renderLoadingProgress(stages[index][0], index, stages.length);
      values.push(await api(stages[index][1]));
      if (requestSerial !== state.overviewRequestSerial || state.bookId !== requestedBookId) return;
      renderLoadingProgress(stages[index][0], index + 1, stages.length);
    }
  } catch (error) {
    renderLoadingProgress("读取失败", values.length, stages.length, error.message || "请求失败");
    toast(error.message, true);
    return;
  }
  const [overview, mapLayout, narrativeMemory, knowledgeFacets, concepts, systems, reviewTasks, narrativeStructure] = values;
  if (requestSerial !== state.overviewRequestSerial || state.bookId !== requestedBookId) return;
...
function configureProgress() {
  ...
  const segment = overview.segments[through];
  $("#progress-chapter").textContent = segment?.chapter_title || "无章节";
  $("#progress-count").textContent = `第 ${through + 1} 章 · 共 ${overview.segments.length} 章`;
  $("#progress-count").setAttribute("aria-label", `编辑阅读范围结束片段，当前第 ${through + 1} 段`);
  $("#progress-from-value").textContent = String(from + 1);
  $("#progress-through-value").textContent = String(through + 1);
}
...
function beginProgressEdit() {
  ...
  const finish = async (commit) => {
    ...
    if (next - 1 < Number(state.readingFrom || 0)) {
      toast("结束片段不能早于起始片段", true);
      return;
    }
    count.textContent = `第 ${next} 章 · 共 ${state.overview.segments.length} 章`;
    await loadOverview(next - 1, false, state.readingFrom);
  };
...
function renderTimeline() {
  ...
  const openThreads = (state.narrativeMemory?.open_threads || []).filter((item) => item.status === "open" && item.actionability === "actionable").slice(0, 8);
  const characterStates = (state.narrativeMemory?.character_states || []).filter((item) => item.goal || item.states?.length).slice(0, 8);
  const threadMarkup = openThreads.map((item) => `<div class="thread-row"><span>${escapeHtml(item.title)}</span><span class="thread-actions">${item.evidence_segment_ids?.[0] ? `<button type="button" class="button button-quiet thread-source" data-segment="${item.evidence_segment_ids[0]}">查看原文</button>` : ""}<button type="button" class="button button-quiet thread-review" data-thread="${escapeHtml(item.thread_key || item.title)}">进入处理</button></span></div>`).join("");
  const memoryPanel = openThreads.length || characterStates.length ? `<details class="narrative-memory"><summary>当前承接记忆${openThreads.length ? ` · ${openThreads.length} 条可处理线索` : ""}</summary><div class="narrative-memory-content">${openThreads.length ? `<article><strong>可处理线索</strong><div class="thread-list">${threadMarkup}</div></article>` : ""}${characterStates.length ? `<article><strong>人物当前状态</strong><p>${characterStates.map((item) => `${escapeHtml(item.name)}：${escapeHtml(item.goal || item.states?.join("、") || "状态已记录")}${item.location_name ? `，位于${escapeHtml(item.location_name)}` : ""}`).join("<br>")}</p></article>` : ""}</div></details>` : "";
  ...
  bindTargets();
  $$(".thread-source").forEach((button) => button.addEventListener("click", () => openSource(Number(button.dataset.segment), "")));
  $$(".thread-review").forEach((button) => button.addEventListener("click", () => {
    state.view = "quality";
    renderView();
    toast("已打开质量检查；请处理对应的证据任务");
  }));
}
...
async function loadStoryContext(event, bookId = state.bookId) {
  ...
  loading.className = "story-knowledge-capsule story-knowledge-loading";
  if (card) {
    loading.innerHTML = '<div class="loading-inline-progress"></div>';
    card.appendChild(loading);
    renderInlineLoading(loading.querySelector(".loading-inline-progress"), "整理当前步骤知识", 0, 1);
  }
  try {
    const context = await api(`/api/books/${bookId}/story-context/${event.id}${readingQuery(state.readingFrom, state.readingThrough)}`);
    ...
    if (loading.isConnected) renderInlineLoading(loading.querySelector(".loading-inline-progress") || loading, "整理当前步骤知识失败", 1, 1, error.message);
...
async function loadArchivedWorldNotes() {
  ...
  if (!host) return;
  renderInlineLoading(host, "读取归档世界信息", 0, 1);
  try {
    ...
    renderInlineLoading(host, "读取归档世界信息失败", 1, 1, error.message, loadArchivedWorldNotes);
  }
}
...
function renderDatabase(query = state.knowledgeQuery || "", category = state.knowledgeCategory || "all") {
  ...
  if (Number(state.conceptsBookId) !== Number(state.bookId)) {
    $("#view-panel").innerHTML = panelHead("知识库", "正在切换到当前作品") + '<div id="database-loading"></div>';
    renderInlineLoading($("#database-loading"), "读取知识结构", 0, 1);
    return;
  }
...
async function openConceptDetails(conceptId) {
  ...
  renderInlineLoading($("#inspector-body"), "读取原子事实", 0, 2);
  ...
  try {
    const windowQuery = readingQuery(state.readingFrom, state.readingThrough);
    const [claims, revisions] = await Promise.all([
      api(`/api/books/${requestedBookId}/knowledge-claims?concept_id=${conceptId}${windowQuery ? `&${windowQuery.slice(1)}` : ""}`),
      api(`/api/books/${requestedBookId}/knowledge-revisions?target_type=concept&target_id=${conceptId}&limit=20${windowQuery ? `&${windowQuery.slice(1)}` : ""}`),
    ]);
    ...
    if (requestSerial === state.inspectorRequestSerial && requestedBookId === Number(state.bookId)) {
      renderInlineLoading($("#inspector-body"), "读取原子事实失败", 2, 2, error.message, () => openConceptDetails(conceptId));
    }
  }
}
...
async function refreshControlPlane(silent = false) {
  ...
  const requestedBookId = state.bookId;
  if (!silent) {
    $("#view-panel").innerHTML = panelHead("分析设置与记录", "正在读取分析规则、模型状态、成本和运行记录") + '<div id="control-plane-loading"></div>';
    renderInlineLoading($("#control-plane-loading"), "读取分析设置与记录", 0, 1);
  }
  try {
    ...
    $("#view-panel").innerHTML = panelHead("分析设置与记录", "读取失败") + '<div id="control-plane-loading"></div>';
    renderInlineLoading($("#control-plane-loading"), "读取分析设置与记录失败", 1, 1, error.message, () => refreshControlPlane(false));
  }
}
...
async function openInspector(type, id) {
  ...
  renderInlineLoading($("#inspector-body"), "读取原文证据", 0, 1);
  $("#inspector").classList.add("open");
  $(".app-shell").classList.add("inspector-open");
  $("#inspector").setAttribute("aria-hidden", "false");
  $("#inspector").removeAttribute("inert");
  $("#scrim").hidden = !window.matchMedia("(max-width: 1100px)").matches;
  try {
    const evidence = await api(`/api/evidence/${type}/${id}${readingQuery(state.readingFrom, state.readingThrough)}`);
    ...
    renderInlineLoading($("#inspector-body"), "读取原文证据失败", 1, 1, error.message, () => openInspector(type, id));
  }
}
...
async function generateRecordDraft(type, id) {
  ...
  renderInlineLoading(preview, "生成证据受限草稿", 0, 1);
  try {
    ...
    renderInlineLoading(preview, "生成证据受限草稿失败", 1, 1, error.message, () => generateRecordDraft(type, id));
  }
}
...
async function openSource(segmentId, quote) {
  const sourceLoading = $("#source-text");
  renderInlineLoading(sourceLoading, "读取原文片段", 0, 1);
  try {
    const source = await api(`/api/segments/${segmentId}${readingQuery(state.readingFrom, state.readingThrough)}`);
    ...
    renderInlineLoading(sourceLoading, "读取原文片段失败", 1, 1, error.message, () => openSource(segmentId, quote));
    toast(error.message, true);
  }
}
...
async function refreshAnalysisEstimate() {
  ...
  renderInlineLoading(estimateBox, "预测分析费用", 0, 1);
  ...
  try {
    ...
    state.analysisEstimate = null;
    renderInlineLoading(estimateBox, "预测分析费用失败", 1, 1, error.message, refreshAnalysisEstimate);
...
async function searchBook() {
  ...
  renderInlineLoading($("#search-results"), "搜索当前阅读范围", 0, 1);
  openDialog("#search-dialog", "#search-close");
  try {
    const searchWindow = readingQuery(state.readingFrom, state.readingThrough);
    const results = await api(`/api/books/${state.bookId}/search?q=${encodeURIComponent(query)}${searchWindow ? `&${searchWindow.slice(1)}` : ""}`);
    ...
    renderInlineLoading($("#search-results"), "搜索失败", 1, 1, error.message, searchBook);
  }
}
...
$("#progress-slider").addEventListener("change", (event) => {
  const through = Number(event.target.value);
  const from = Math.min(through, Number($("#progress-from-slider")?.value || 0));
  $("#progress-from-slider").value = String(from);
  state.readingFrom = from;
  state.readingThrough = through;
...
$("#progress-from-slider").addEventListener("input", (event) => {
  const through = Number($("#progress-slider").value || 0);
  const from = Math.min(through, Number(event.target.value));
  event.target.value = String(from);
  state.readingFrom = from;
  $("#progress-from-value").textContent = String(from + 1);
});
$("#progress-from-slider").addEventListener("change", (event) => {
  const through = Number($("#progress-slider").value || 0);
  const from = Math.min(through, Number(event.target.value));
  state.readingFrom = from;
  state.readingThrough = through;
