...
function installMap3DRegionMeshes(graph, centerX, centerY, attempt = 0) {
  ...
  regions.forEach((region, regionIndex) => {
    ...
    const defaultThickness = region.kind === "evidence_containment" ? 28 : 18;
    ...
    const material = new MaterialConstructor({
      ...
      opacity: state.map3DVolumeMode === "layer" && !["current", "ancestor"].includes(status) ? 0.015
        : active ? (region.kind === "evidence_containment" ? 0.48 : 0.38)
...
async function loadOverview(throughSegment = null, silent = false) {
  ...
  const [overview, benchmarks, benchmarkCandidates, mapLayout, narrativeMemory, knowledgeFacets, concepts, systems, evaluationProgress] = await Promise.all([
    ...
    api("/api/eval-suites/release-gate"),
  ]);
  ...
  state.evaluationProgress = evaluationProgress;
...
function renderCollaboration() {
  ...
  if (controlPlane) {
    const corpusMetric = [...controlPlane.querySelectorAll(".evaluation-grid > span")].find((item) => item.textContent.trim().startsWith("真实作品"));
    if (corpusMetric) corpusMetric.innerHTML = `真实作品 <strong>${Number(evaluation.book_count || 0)}/12 · ${Number(evaluation.books_below_minimum_cases || 0)} 本不足 25 条</strong>`;
...
function renderBenchmarkPanel() {
  ...
  const evaluation = state.evaluationProgress || {};
  const globalConfirmed = Number(evaluation.confirmed_cases || 0);
  const validHoldouts = Number(evaluation.holdout_cases || 0);
  ...
  const indexedCandidateRows = state.benchmarkCandidates.map((item) => {
    ...
    const provenance = evidence.length
      ...
      : "本地检查候选 · 未附逐字引文";
    return `<article class="benchmark-case candidate"><div><span class="benchmark-status pending">待你核对</span><strong>${escapeHtml(item.subject)}</strong><small>${escapeHtml(benchmarkTypeLabels[item.case_type] || item.case_type)} · ${escapeHtml(chapterForSegment(item.source_segment))}${item.critical ? " · 关键事实" : " · 普通事实"}</small><p><b>系统准备的答案：</b>${escapeHtml(benchmarkExpectedText(item.case_type, item.expected))}</p><p class="benchmark-note">${provenance}</p><p class="benchmark-note">${escapeHtml(item.note)}</p></div><div class="benchmark-actions"><button class="button button-quiet benchmark-candidate-source" data-segment="${item.source_segment}" type="button">先看原文</button><button class="button button-primary benchmark-candidate-accept" data-id="${item.id}" type="button">答案正确</button><button class="button button-quiet benchmark-candidate-seal" data-id="${item.id}" type="button">正确并作为密封题</button><button class="button button-danger benchmark-candidate-reject" data-id="${item.id}" type="button">答案不对</button></div></article>`;
  }).join("");
  ...
  const candidatePanel = `<details id="benchmark-candidate-panel" class="benchmark-form benchmark-review-queue" open><summary>开始人工核对 · 当前书籍待处理 ${state.benchmarkCandidates.length + seededCandidates.length} 条</summary><div class="benchmark-head"><div><p>操作顺序只有三步；先看原文；再比较系统答案；最后选择“答案正确”“密封题”或“答案不对”；整个过程不调用模型</p></div><button id="benchmark-candidate-refresh" class="button button-quiet" type="button">从现有证据补充题目</button></div><div class="benchmark-list">${candidateRows}</div></details>`;
  const explainer = `<div class="benchmark-explainer"><article><span>人工金标准</span><strong>${globalConfirmed}/300</strong><p>300 道人工看过原文并确认答案的测试题；它验证人物、关系、时间、地点和证据是否正确；这里就是审核入口</p></article><article><span>有效密封案例</span><strong>${validHoldouts}/60</strong><p>从 300 道题中选 60 道作为期末考试；答案确认后隐藏；再由第二次复核确认，才算一条有效密封案例</p></article><article class="benchmark-next-step"><span>你现在怎么做</span><strong>从下面第一题开始</strong><p>每本开放作品计划 25 题；其中 20 题用于日常检查，5 题设为密封题；不需要一次做完</p><button id="benchmark-start-review" class="button button-primary" type="button">审核下一题</button></article></div>`;
  return `<section class="benchmark-panel"><header class="benchmark-head"><div><h3>人工验收中心</h3><p>这是内容准确率的人工考试，不是登录或网页可用性检查；${escapeHtml(summary)}；本地复算费用为 $0</p></div><button id="benchmark-evaluate" class="button button-quiet" type="button">重新本地复算</button></header>${explainer}${candidatePanel}<details class="benchmark-form"><summary>${editing ? "修正或编辑这道题" : "手动新增一道测试题"}</summary><div class="benchmark-form-grid"><label>核对类型<select id="benchmark-case-type">${Object.entries(benchmarkTypeLabels).map(([value, label]) => `<option value="${value}" ${value === caseType ? "selected" : ""}>${escapeHtml(label)}</option>`).join("")}</select></label><label>核对说明<input id="benchmark-case-subject" maxlength="240" value="${escapeHtml(editing?.subject || "")}" placeholder="例如：孙悟空与齐天大圣是同一人物"></label><div id="benchmark-expected-fields" class="benchmark-expected-fields">${benchmarkExpectedFields(caseType, editing?.expected || {})}</div><label>原文章节<select id="benchmark-source-segment">${sourceOptions}</select></label><label>评估集名称<input id="benchmark-suite" maxlength="120" value="${escapeHtml(editing?.suite_name || "real-novel-gold")}" placeholder="例如：real-novel-gold"></label><label>错误类别<input id="benchmark-failure-category" maxlength="120" value="${escapeHtml(editing?.failure_category || "")}" placeholder="例如：亲属关系、时间倒叙或地点方位"></label><label class="benchmark-note-field">原文核对说明<textarea id="benchmark-note" maxlength="1000" rows="3" placeholder="说明你依据哪个原文事实作出判断">${escapeHtml(editing?.note || "")}</textarea></label><label class="benchmark-critical"><input id="benchmark-critical" type="checkbox" ${editing?.critical !== false ? "checked" : ""}> 这是主体关键项；失败时不能通过最终门禁</label><label class="benchmark-critical"><input id="benchmark-holdout" type="checkbox" ${editing?.holdout ? "checked" : ""}> 设为密封题；保存后隐藏答案；完成第二次复核后才计入 60 条</label><div class="benchmark-actions"><button id="benchmark-save" class="button button-primary" type="button">${editing ? "保存并重新核验" : "登记并本地核验"}</button>${editing ? '<button id="benchmark-cancel" class="button button-quiet" type="button">取消编辑</button>' : ""}</div></div></details><div class="benchmark-list">${rows}</div></section>`;
}
...
async function resolveBenchmarkCandidate(candidateId, action, holdout = false) {
  const values = await formAction({
    title: action === "accept" ? (holdout ? "确认并密封这道题" : "确认系统答案正确") : "说明系统答案哪里不对",
    description: action === "accept"
      ? (holdout ? "保存后答案会从普通页面隐藏；还需第二次复核才能计入 60 条有效密封案例" : "确认后会计入 300 条人工金标准")
      : "这次不会计入准确率；你可以随后使用手动新增题目登记正确答案",
    submitLabel: action === "accept" ? (holdout ? "确认并密封" : "确认答案正确") : "标记答案不对",
  ...
  try {
    await api(`/api/benchmark-candidates/${candidateId}/resolve`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action, holdout, note, reviewer_id: "local-owner", reviewer_role: "owner", review_session: `ui-${Date.now()}` }),
    });
    toast(action === "accept" ? (holdout ? "密封题已保存；完成第二次复核后计入 60 条" : "答案已经人工确认并计入 300 条") : "答案已标记为不正确；不会计入准确率");
    await loadOverview(Number($("#progress-slider").value), true);
  } catch (error) {
    toast(error.message, true);
  }
...
function bindBenchmarkPanel() {
  if (state.benchmarkEditingId) $$(".benchmark-form").at(-1)?.setAttribute("open", "");
  ...
  $("#benchmark-start-review")?.addEventListener("click", async () => {
    if (!$("#benchmark-candidate-panel .benchmark-case")) await refreshBenchmarkCandidates();
    const panel = $("#benchmark-candidate-panel");
    if (panel) panel.open = true;
    $("#benchmark-candidate-panel .benchmark-case")?.scrollIntoView({ behavior: "smooth", block: "center" });
  });
  ...
  $$(".benchmark-candidate-accept").forEach((button) => button.addEventListener("click", () => {
    resolveBenchmarkCandidate(Number(button.dataset.id), "accept", false);
  }));
  $$(".benchmark-candidate-seal").forEach((button) => button.addEventListener("click", () => {
    resolveBenchmarkCandidate(Number(button.dataset.id), "accept", true);
  }));
...
function renderQuality() {
  ...
  const accuracyDetail = quality.accuracy_gate_required && quality.benchmark_cases < 25
    ? `当前书籍已经人工确认 ${quality.benchmark_cases}/25 条`
