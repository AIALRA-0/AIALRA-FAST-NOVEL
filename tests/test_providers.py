...
@pytest.mark.parametrize(
    ("status_code", "payload", "expected_status", "expected_reachable"),
    [
        (200, {"data": [{"id": "deepseek-v4-flash"}]}, "connected", True),
        (200, {"data": [{"id": "other-model"}]}, "model_unavailable", True),
        (401, {}, "auth_failed", True),
        (503, {}, "service_error", False),
    ],
)
def test_provider_probe_maps_directory_outcomes_without_response_body(
    ...
    class FakeResponse:
        ...
        def raise_for_status(self) -> None:
            if self.status_code >= 400 and self.status_code < 500 and self.status_code not in {401, 403}:
                raise httpx.HTTPStatusError("fake", request=httpx.Request("GET", "https://example.invalid"), response=httpx.Response(self.status_code))
    ...
    monkeypatch.setattr("app.providers.httpx.AsyncClient", FakeClient)
    from app.providers import probe_provider

    result = asyncio.run(probe_provider(
        "deepseek", api_key="test-secret",
...
def test_provider_probe_maps_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    """目录请求超时不会被误报为已连接。"""
    ...
    result = asyncio.run(probe_provider(
        "moonshot", api_key="test-secret",
        base_url="https://api.moonshot.cn", model="kimi-k2.5",
    ))
