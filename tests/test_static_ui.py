...
def test_v294_human_acceptance_language_and_wrapped_map_labels_are_actionable() -> None:
    script = _text("static/app.js")
    styles = _text("static/styles.css")
    assert "人工验收中心" in script
    assert "人工金标准" in script and "/300" in script
    assert "有效密封案例" in script and "/60" in script
    assert 'id="benchmark-start-review"' in script
    assert "benchmark-candidate-seal" in script
    ...
    assert ".benchmark-explainer" in styles
