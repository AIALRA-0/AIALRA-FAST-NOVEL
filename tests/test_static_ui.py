...
def test_release_version_is_consistent_across_state_api_and_page() -> None:
    ...
    assert f'APP_VERSION = "{state["release"]}"' in _text("app/main.py")
...
def test_monochrome_ui_has_large_controls_and_visible_focus() -> None:
    ...
    assert "--control-height: 44px" in styles
    assert "min-height: var(--control-height)" in styles
...
def test_v29_shared_select_progress_and_quality_workbench_contract() -> None:
    ...
    assert "novel-atlas-map-camera-mode" in script
    assert 'state.mapCameraMode === "world"' in script
...
def test_v295_internal_evaluation_is_absent_from_reader_ui() -> None:
    script = _text("static/app.js")
    styles = _text("static/styles.css")
    assert "人工验收中心" not in script
    assert "人工金标准" not in script
    assert "有效密封案例" not in script
    assert 'data-tab="gold"' not in script
    assert 'data-panel="gold"' not in script
    assert "/api/books/${requestedBookId}/benchmarks" not in script
    assert "/api/eval-suites/release-gate" not in script
    assert "reportLanguageLabels" in script
    assert "book-report-language-edit" in script
    assert "function mapLabelLines" in script
    assert "svgTextLines(regionLines" in script
    assert "link.current ? 9 : 6.5" in script
    assert "这里只显示真正影响当前书籍的问题" in script
    assert ".knowledge-sidebar .world-create-form" in styles
    assert "已分析原文片段" in script
...
def test_v291_map_controls_flow_without_absolute_overlap_and_regions_stay_visible() -> None:
    ...
    assert 'mapLayout.validation_state === "invalid"' in script
    assert "语义世界图暂未显示" in script
    assert "地点关系与当前布局不一致" in script


def test_v297_settings_cards_use_shared_spacing_tokens() -> None:
    script = _text("static/app.js")
    styles = _text("static/styles.css")
    assert 'data-layout-contract="independent-cards"' in script
    assert "--space-4: 16px" in styles
    assert ".settings-card-grid" in styles
    assert "gap: var(--space-4)" in styles
    assert ".control-section.two-column" not in styles
