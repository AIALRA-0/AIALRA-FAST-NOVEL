...
test("2D 与 3D 地图共享编年状态并支持快速连续切换", async ({ page }) => {
  ...
  await expect(page.locator(".brand span")).toContainText("2.9.4");
...
test("2.9 统一选择器、防剧透输入和质量任务页可直接操作", async ({ page }) => {
  ...
  await page.locator('.quality-tab[data-tab="gold"]').click();
...
test("2.9.4 人工验收入口、生成语言和概念表单可以直接使用", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  await page.goto(baseURL, { waitUntil: "networkidle" });

  await page.locator('.nav-item[data-view="quality"]').click();
  await page.locator('.quality-tab[data-tab="gold"]').click();
  await expect(page.locator(".benchmark-explainer")).toBeVisible();
  await expect(page.locator(".benchmark-explainer")).toContainText("人工金标准");
  await expect(page.locator(".benchmark-explainer")).toContainText("有效密封案例");
  await expect(page.locator("#benchmark-start-review")).toBeVisible();
