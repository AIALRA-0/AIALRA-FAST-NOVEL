...
def test_health_and_readiness_endpoints(tmp_path: Path) -> None:
    ...
    with client_for(tmp_path) as client:
        ...
        assert health == {"status": "ok", "version": "2.9.7-rc.3"}
...
def test_reading_window_filters_end_and_marks_context_at_start(tmp_path: Path) -> None:
    """所有派生对象共享双向窗口；起点前只作为明确上下文保留。"""

    with client_for(tmp_path) as client:
        book_id = next(book["id"] for book in client.get("/api/books").json() if book["title"] == "雾川行记 · 演示")
        response = client.get(
            f"/api/books/{book_id}/overview",
            params={"from_segment": 1, "through_segment": 3},
        )
        assert response.status_code == 200
        payload = response.json()
        assert payload["reading_window"] == {
            "from_segment": 1,
            "through_segment": 3,
            "spoiler_ceiling": 3,
            "total_segments": 5,
        }
        assert all(int(item["first_segment"]) <= 3 for item in payload["events"])
        assert any(item.get("context_only") for item in payload["events"] if int(item["first_segment"]) < 1)
        memory = client.get(
            f"/api/books/{book_id}/narrative-memory",
            params={"from_segment": 1, "through_segment": 3},
        ).json()
        assert memory["reading_window"] == payload["reading_window"]
        assert all(int(item["first_segment"]) <= 3 for item in memory["recent_scenes"])
        invalid = client.get(
            f"/api/books/{book_id}/overview",
            params={"from_segment": 4, "through_segment": 2},
        )
...
def test_unconfigured_provider_probe_is_explicit_and_secret_free(tmp_path: Path) -> None:
    ...
    with client_for(tmp_path) as client:
        response = client.post("/api/providers/deepseek/probe")
        assert response.status_code == 200
        payload = response.json()
...
def test_v292_map_layout_exposes_named_regions_and_external_label_geometry(tmp_path: Path) -> None:
    ...
    with client_for(tmp_path) as client:
        ...
        assert atlas["layout_version"] == "semantic-atlas-v2.9.7-constraint-gate1"
        assert atlas["validation_state"] == "valid"
        assert atlas["failed_relations"] == []
        assert atlas["constraint_summary"]["failed_relation_count"] == 0
...
def test_explicitly_near_places_share_an_evidence_region_and_remain_nearby(tmp_path: Path) -> None:
    """明确相邻的地点不能被故事分区拆开并推到地图两端。"""

    with client_for(tmp_path) as client:
        book_id = next(book["id"] for book in client.get("/api/books").json() if book["title"] == "雾川行记 · 演示")
        with transaction(main.settings.database_path) as connection:
            first = int(connection.execute(
                "INSERT INTO entities(book_id, kind, name, summary, importance, first_segment) VALUES (?, 'place', '测试桃园', '测试庄园后方的桃园', 0.9, 0)",
                (book_id,),
            ).lastrowid)
            second = int(connection.execute(
                "INSERT INTO entities(book_id, kind, name, summary, importance, first_segment) VALUES (?, 'place', '测试庄园', '桃园的参照地点', 0.8, 0)",
                (book_id,),
            ).lastrowid)
            connection.execute(
                "INSERT INTO place_relations(book_id, source_entity_id, target_entity_id, relative_position, summary, confidence, first_segment) VALUES (?, ?, ?, 'near', '测试桃园位于测试庄园后方', 0.98, 0)",
                (book_id, first, second),
            )

        atlas = client.get(f"/api/books/{book_id}/map-layout", params={"through_segment": 4}).json()
        nodes = {int(node["id"]): node for node in atlas["nodes"]}
        distance = math.hypot(
            float(nodes[first]["x"]) - float(nodes[second]["x"]),
            float(nodes[first]["y"]) - float(nodes[second]["y"]),
        )
        matching = [
            region for region in atlas["regions"]
            if {first, second} <= {int(node_id) for node_id in region["node_ids"]}
        ]
        assert distance <= 280.0
        assert len(matching) == 1
        assert matching[0]["kind"] == "evidence_proximity"
        assert set(matching[0]["representative_node_ids"]) == {first, second}
        assert not any(
            {first, second}.intersection(int(node_id) for node_id in region["node_ids"])
            for region in atlas["regions"] if region["kind"] == "topological_cluster"
        )
...
def test_v29_ui_issue_ledger_is_actionable_and_verifiable(tmp_path: Path) -> None:
    ...
    with client_for(tmp_path) as client:
        created = client.post(
            "/api/ui-issues",
            json={
                "page_key": "atlas-3d", "component_key": "map.region-labels", "issue_class": "overflow",
                "viewport": "1366x768@125%", "detected_environment": "Chromium 1366x768 125%", "severity": "high",
                "summary": "区域标签超出画布", "reproduction": "打开真实长篇并切换 3D",
                "root_cause": "标签边界未纳入全图范围", "acceptance": "全部标签可恢复到画布范围",
                "regression_test": "tests/e2e_ui.spec.js", "regression_case": "map labels remain in bounds",
            },
        )
        assert created.status_code == 201
        issue_id = created.json()["id"]
        assert created.json()["component_key"] == "map.region-labels"
        assert client.get("/api/ui-issues", params={"status": "open"}).json()[0]["id"] == issue_id
        fixed = client.patch(f"/api/ui-issues/{issue_id}", json={"status": "fixed"})
        assert fixed.status_code == 200
        assert fixed.json()["closed_at"] is None
        missing_evidence = client.patch(f"/api/ui-issues/{issue_id}", json={"status": "verified"})
        assert missing_evidence.status_code == 422
        verified = client.patch(f"/api/ui-issues/{issue_id}", json={
            "status": "verified", "verified_environment": "Chromium 1366x768 125%",
            "screenshot_path": "acceptance/atlas-3d-1366.png",
        })
