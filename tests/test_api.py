...
def test_health_and_readiness_endpoints(tmp_path: Path) -> None:
    ...
    with client_for(tmp_path) as client:
        ...
        assert health == {"status": "ok", "version": "2.9.3-rc.1"}
        readiness = client.get("/readyz")
        assert readiness.status_code == 200
        assert readiness.json() == {"status": "ready", "version": "2.9.3-rc.1"}
...
def test_v292_map_layout_exposes_named_regions_and_external_label_geometry(tmp_path: Path) -> None:
    ...
    with client_for(tmp_path) as client:
        ...
        assert atlas["layout_version"] == "semantic-atlas-v2.9.3-occupancy-volume1"
...
def test_real_model_analysis_requires_gold_standard_before_final_quality_pass(tmp_path: Path) -> None:
    """真实模型任务没有足够人工金标准时只能通过结构检查，不能宣称达到最终准确率。"""
    ...
    with client_for(tmp_path) as client:
        ...
        assert overview["quality"]["quality_gate_passed"] is False
        assert any("金标准" in issue["title"] for issue in overview["quality"]["issues"])
