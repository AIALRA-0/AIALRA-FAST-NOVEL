...
def test_control_plane_exposes_settings_without_internal_evaluation_payload(tmp_path: Path) -> None:
    """普通设置页应保留提示词和历史反馈，但不返回内部评测数据。"""
    ...
    with client_for(tmp_path) as client:
        ...
        body = response.json()
        assert body["contract"]["quality"]["quote_integrity_percent"] == 100
        assert "overall_holdout_percent" not in body["contract"]["quality"]
        ...
        assert "evaluation_progress" not in body
        assert "suite_version" not in body
        assert all("benchmark" not in route for route in body["model_routes"])
...
def test_book_report_language_is_visible_in_prompt_and_changes_its_hash(tmp_path: Path) -> None:
    """每本书可以选择报告语言；原文证据规则仍保留在提示词中。"""

    with client_for(tmp_path) as client:
        book_id = demo_book(client)["id"]
        original = client.get(f"/api/books/{book_id}/prompt-bundles/extraction").json()
        updated = client.patch(f"/api/books/{book_id}", json={"report_language": "en"})
        assert updated.status_code == 200
        assert updated.json()["report_language"] == "en"
        rendered = client.get(f"/api/books/{book_id}/prompt-bundles/extraction").json()
        language_layer = next(layer for layer in rendered["layers"] if layer["key"] == "output_language")
        assert language_layer["value"] == "en"
        assert "reader-facing summary" in rendered["system_prompt"]
        assert "never translate or rewrite quotations" in rendered["system_prompt"]
        assert rendered["prompt_hash"] != original["prompt_hash"]
...
def test_release_gate_discloses_missing_scale_without_claiming_ninety_five_percent(tmp_path: Path) -> None:
    ...
    with client_for(tmp_path) as client:
        ...
        assert body["minimum_books"] == 12
        assert body["minimum_cases_per_book"] == 25
