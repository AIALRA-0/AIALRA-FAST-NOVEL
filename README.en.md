<div align="center">
  ...
  <p><strong>2.9.7-rc.3 release candidate</strong> · <a href="README.md">中文</a> · <a href="docs/CURRENT_STATE.md">Current verification</a> · <a href="docs/DEPLOYMENT.md">Deployment and rollback</a></p>
</div>
...
The current 2.9.7-rc.3 local candidate gate reports

- 162 Python tests, with 161 passing and 1 environment-dependent skip for the Windows DPAPI protected path
...
- 16 passing Chromium flows out of 19, with 3 conditional flows requiring local extended fixtures not enabled in this run
- local API and browser coverage for library switching, reading windows, maps, chronology, relationships, knowledge, and provider status across 12 open works
...
- reverse validation of 64 containment, 19 proximity, and 5 directional relations across 12 analyzed open works; the semantic atlas stops rendering if the evidence gate fails
...
The real-novel acceptance database and screenshots remain local and are excluded from the public repository. This candidate has not converted local evidence into a claim that GitHub or the VPS is complete

These numbers describe fixed samples only and do not imply a fixed accuracy rate for arbitrary novels. Internal regression samples are maintained by the project workflow, remain outside the reader interface, and never require readers to complete a test bank

The ordinary quality page only shows missing evidence, identity conflicts, relationship gaps, temporal contradictions, unresolved locations, and map failures. Safe cases are handled automatically; uncertain cases retain their evidence and an explicit action
